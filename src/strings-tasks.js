...
function orderAlphabetically(str) {
  ...
  if (!/\d/.test(str)) {
    letter.sort((a, b) => a.charCodeAt(a) - 64 - (b.charCodeAt(b) - 64));
    return letter.join('');
  }
  const digits = letter.filter((char) => /\d/.test(char));
  const nonDigits = letter.filter((char) => !/\d/.test(char));
  const sortedChars = digits.concat(nonDigits);
...
function countVowels(str) {
  let counter = 0;
  const vls = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  for (let i = 0; i < str.length; i += 1) {
    if (vls.includes(str[i])) {
      counter += 1;
    }
  }
  return counter;
...
function isPalindrome(str) {
  const sentenceOne = str.replace(/[,\s!?]/g, '').toLowerCase();
  const sentenceTwo = str
    .replace(/[,\s!?]/g, '')
    .toLowerCase()
    .split('')
    .reverse()
